function T = stack_of_spirals_jump(R,Nradial,Nz,fov,resolution, pf)
...
for i=1:(floor(length(kz)/2))
    T2(i)=T(floor(length(kz)/2)-i+1);
    if T(floor(length(kz)/2)-i+1).in_out==-1
        T2(i).K=T2(i).K(1:length(T2(i).K)/2,:);
        T2(i).G=T2(i).G(1:length(T2(i).G)/2,:);
    else
        T2(i).K=T2(i).K((1+length(T2(i).K)/2):end,:);
        T2(i).G=T2(i).G((1+length(T2(i).G)/2):end,:);
    end

    T2(floor(length(kz)/2)+i)=T(i);
    if T(i).in_out==1
        T2(floor(length(kz)/2)+i).K=T(i).K(1:length(T(i).K)/2,:);
        T2(floor(length(kz)/2)+i).G=T(i).G(1:length(T(i).G)/2,:);
    else
        T2(floor(length(kz)/2)+i).K=T(i).K((1+length(T(i).K)/2):end,:);
        T2(floor(length(kz)/2)+i).G=T(i).G((1+length(T(i).G)/2):end,:);
    end
end

tmp=T((1+floor(length(kz)/2)):length(kz));
for i=1:length(tmp)
    T3(i)=tmp(i);
    if tmp(i).in_out==1
        T3(i).K=tmp(i).K(1:length(tmp(i).K)/2,:);
        T3(i).G=tmp(i).G(1:length(tmp(i).G)/2,:);
    else
        T3(i).K=tmp(i).K((1+length(tmp(i).K)/2):end,:);
        T3(i).G=tmp(i).G((1+length(tmp(i).G)/2):end,:);
    end

    T3(length(tmp)+i)=tmp(length(tmp)-i+1);
    if tmp(length(tmp)-i+1).in_out==-1
        T3(length(tmp)+i).K=tmp(length(tmp)-i+1).K(1:length(tmp(length(tmp)-i+1).K)/2,:);
        T3(length(tmp)+i).G=tmp(length(tmp)-i+1).G(1:length(tmp(length(tmp)-i+1).G)/2,:);
    else
        T3(length(tmp)+i).K=tmp(length(tmp)-i+1).K((1+length(tmp(length(tmp)-i+1).K)/2):end,:);
        T3(length(tmp)+i).G=tmp(length(tmp)-i+1).G((1+length(tmp(length(tmp)-i+1).G)/2):end,:);
    end
end
for i=1:length(T2)
    T(i,1,1)=T2(i);
