function T = stack_of_spirals_multi_seg_z(R,Nradial,Nz,fov,resolution)
...
i = 1;
while kz(i) < k_max
    kz(i+1) = min(kz(i) + (Rz_min + kz(i)/k_max * (Rz_max - Rz_min))/fov(1), k_max);
...
for iz=1:Nz
    kz1=kz(iz:Nz:end);
    kr_max1=kr_max(iz:Nz:end);
    % in_out = 1 - mod(size(kz1,2)-1, 4);
    in_out = -1;
    for element = 1:length(kz1)
        Rmin = (Rrad_min + abs(kz1(element)/k_max) * (Rrad_max - Rrad_min));
        Rmax = Rrad_max;
        T(element, iz, 1) = single_element_spiral(kz1(element), kr_max1(element), Rmin, Rmax, fov(1), in_out, SYSTEM);
...
for k=1:length(T2)
    ...
    T2(k).K=[temp;T2(k).K];
    T2(k).G=[temp;T2(k).G];
    T2(k).index=T2(k).index+200;
end
T((end+1):(end+length(T2)))=T2;
%%
