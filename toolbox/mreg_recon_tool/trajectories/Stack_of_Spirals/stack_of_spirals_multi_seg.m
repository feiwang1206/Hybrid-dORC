function T = stack_of_spirals_multi_seg(R,Nradial,Nz,fov,resolution)
...
n=1;
for i=1:size(T,1)
    lth=length(T(i,1,1).K);
    lth0=lth0+lth;
    if lth0>1000
        if n==1
            group{n}=1:i;
        else
            group{n}=(group{n-1}(end)+1):i;
        end
        n=n+1;
        lth0=0;
    end
...
for ir=1:Nradial
    for iz=1:length(group)
        for i=1:length(group{iz})
            T1(i,iz,ir)=T(group{iz}(i),1,ir);
        end
    end
end
T=T1;
