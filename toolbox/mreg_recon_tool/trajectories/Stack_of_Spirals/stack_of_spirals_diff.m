...
for iradial=1:Nradial
    for iz=1:Nz
        ...
        T(1,iz,iradial).index(1) = length(T(1,iz,iradial).G) - length(temp.G)+0;
    end
end
...
for k=1:length(T)
    T(k).index(2) = length(T(k).G)+0;
    T(k)=trajectStruct_rampDown(T(k));
    T(k).K((end+1):(end+200),:)=0;
    T(k).G((end+1):(end+200),:)=0;
...
Td(1,iz,1).G(501:1000,xyz)=-Td(1,iz,1).G(1:500,xyz);
...
%     for j=(1:Nradial)-1
%         for k=1:2
%             T(i*2*Nradial+j*2+k)=T0(j+1);
%             % tmp=[b(i+1)*Td(1).G;(-1)^(k-1)*T0(j+1).G];
%             tmp=[(-1)^(k-1)*T0(j+1).G];
%             T(i*2*Nradial+j*2+k).G=tmp;
%             T(i*2*Nradial+j*2+k).K=gamma*cumsum(tmp)*1e-5;
%         end
%     end
% end
%%
...
for i=(1:length(b))-1
    for j=(1:Nradial)
        T(i*Nradial+j)=T0(j);
        tmp=[b(i+1)*Td(1).G;T0(j).G];
        T(i*Nradial+j).G=tmp;
        T(i*Nradial+j).K=gamma*cumsum(tmp)*1e-5;
        T(i*Nradial+j).index=T(i*Nradial+j).index+1000;
    end
end
