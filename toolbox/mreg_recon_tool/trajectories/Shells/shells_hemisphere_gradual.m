function T=shells_hemisphere_gradual(R,fov,resolution,Npolar,Nradial,varargin)
...
len=0;
for ii=1:length(T)
    len=len+length(T(ii).K);
end
basek=min(shell_radii);%0;%deltaK_full;%
dkr=(max(shell_radii)-basek)/len;
nn=0;
for ii=1:length(T)
    for jj=1:length(T(ii).K)
        sos=sqrt(T(ii).K(jj,1)^2+T(ii).K(jj,2)^2+T(ii).K(jj,3)^2);
        ratio=((len-nn)*dkr+basek)/sos;
        T(ii).K(jj,:)=T(ii).K(jj,:)*ratio;
        nn=nn+1;
    end
end
% dkr=(max(shell_radii))/len;
% nn=0;
% for ii=1:length(T)
%     for jj=1:length(T(ii).K)
%         sos=sqrt(T(ii).K(jj,1)^2+T(ii).K(jj,2)^2+T(ii).K(jj,3)^2);
%         ratio=((len-nn)*dkr)/sos;
%         T(ii).K(jj,:)=T(ii).K(jj,:)*ratio;
%         nn=nn+1;
%     end
% end
for k=1:length(T)
    [~, T(k).G] = gradient(T(k).K,1e-5);
    T(k).G = T(k).G/gamma;
end
