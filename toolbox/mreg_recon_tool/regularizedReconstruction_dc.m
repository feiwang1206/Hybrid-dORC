function [z, z_iter] = regularizedReconstruction_dc(A,b,lambda,varargin)
...
if linear_flag==0 % use nonlinear conjugate gradient
    ...
    for k=1:length(I_fhandle)
        ...
        if I_all(n+1)==I_all(n)+2
            ...
            for l=2:(I_all(n+1)-I_all(n)-1)
                % R{counter} = @(z) varargin{I_fhandle(k)+1}^2*( varargin{I_all(n)+l}' * (varargin{I_all(n)+l} * (z-z0)));
                R{counter} = @(z) lambda^2*( varargin{I_all(n)+l}' * (varargin{I_all(n)+l} * (z-z0)));
                % R{counter} = @(z) lambda^2*((z-z0));
                counter = counter + 1;
            end
        end
    end
    ...
    if nargout==1
        ...
        % z = DeepestGradient(S,q,tol,maxit,z0,verbose_flag,newfigure);
    else
        [z,z_iter] = conjugateGradient(S,q,tol,maxit,z0,verbose_flag,newfigure);
        % [z,z_iter] = DeepestGradient(S,q,tol,maxit,z0,verbose_flag,newfigure);
    end    
end
