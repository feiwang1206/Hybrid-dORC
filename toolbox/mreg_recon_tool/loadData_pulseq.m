function [out, hdr] = loadData_pulseq(filename,mshot,t)
...
if nargin<3
    out = squeeze(twx.image());
    out=reshape(out,[size(out,1) size(out,2) mshot size(out,3)/mshot]);
else
    out = squeeze(twx.image());
    out=reshape(out,[size(out,1) size(out,2) mshot size(out,3)/mshot]);
    out=out(:,:,:,t);
...
%         hdr.TotalScanTimeSec = twx.hdr.MeasYaps.lTotalScanTimeSec;
        ...
        if(cast(version,'double')>67) %if sofware version D or later
            hdr.trajectory = twx.hdr.MeasYaps.sWipMemBlock.tFree{1};
