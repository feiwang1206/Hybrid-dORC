...
x_TE = squeeze(twx.image(:,:,:,:,:,:,:,:,:,[ceil(idx_k0/size(twx.image(),1))]));
...
ref_idx = 41;% Steady-state time frame to use as reference
...
data_TN=data_TN(:,1:Ni*floor(size(data_TN,2)/Ni));
data_TE=data_TE(:,1:Ni*floor(size(data_TE,2)/Ni));
