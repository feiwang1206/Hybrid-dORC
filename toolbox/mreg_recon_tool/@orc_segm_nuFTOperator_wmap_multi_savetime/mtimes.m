...
if strcmp(class(A),'orc_segm_nuFTOperator_wmap_multi_savetime')
    ...
    if A.adjoint
        ...
        for m=1:length(A.interp_filter)
            ...
            if length(A.imageDim)==3
                Q = Q + x.*conj(repmat(1i*A.t(m).*A.recon.*A.wmap(:,:,:,m),[1 1 1 A.numCoils]));
            else
                Q = Q + x.*conj(repmat(1i*A.t(m).*A.recon.*A.wmap(:,:,m),[1 1 A.numCoils]));
            end
        end
        % B=B.*A.T';
        % for m=1:length(A.interp_filter)
        ...
        %     end
        %     if length(A.imageDim)==3
        %         Q = Q + x.*conj(repmat(1i*A.recon.*A.wmap(:,:,:,m),[1 1 1 A.numCoils]));
        %     else
        %         Q = Q + x.*conj(repmat(1i*A.recon.*A.wmap(:,:,m),[1 1 A.numCoils]));
        %     end
        % end
...
%         Q = sum(Q,4)/sqrt(prod(A.imageDim))/length(A.tADC);
        ...
        if length(A.imageDim)==3
            ...
            for m=1:length(A.interp_filter)
                % x = repmat(1i*A.t(m).*A.recon.*A.wmap(:,:,:,m).*B,[1 1 1 A.numCoils]).*A.sensmaps_scale;
                x = B.*repmat(1i*A.t(m).*A.recon.*A.wmap(:,:,:,m),[1 1 1 A.numCoils]);
            ...
            Q = zeros(A.trajectory_length,A.numCoils);
            for m=1:length(A.interp_filter)
                x = B.*repmat(1i*A.t(m).*A.recon.*A.wmap(:,:,m),[1 1 A.numCoils]);
