function [z, z_iter] = regularizedReconstruction_wmap(A,b,varargin)
...
if linear_flag==0 % use nonlinear conjugate gradient
    ...
    if nargout==1
        [z] = nonlinearConjugateGradient_wmap(f,df,z0,tol,maxit,cg_method,verbose_flag,reference_norm);
    else
        [z, z_iter] = nonlinearConjugateGradient_wmap(f,df,z0,tol,maxit,cg_method,verbose_flag,reference_norm);
    end
    ...
    if nargout==1
        z = conjugateGradient(S,q,tol,maxit,zeros([size(A,2), 1]),verbose_flag,newfigure);
    else
        [z,z_iter] = conjugateGradient(S,q,tol,maxit,zeros([size(A,2), 1]),verbose_flag,newfigure);
    end    
end
