...
if linear_flag==0 % use nonlinear conjugate gradient
    ...
    for iter=1:200
        [~,re,reR] = f(z);
        dfz = df(z,re,reR);
        lam=l2norm(dfz{1})/l2norm(dfz{2})/1000;
        z=z-dfz{1}-lam^2*dfz{2};
    end
    ...
    for k=1:length(I_fhandle)
        ...
        if I_all(n+1)==I_all(n)+2;
            ...
            for l=2:(I_all(n+1)-I_all(n)-1)
                R{counter} = @(z) 1*( varargin{I_all(n)+l}' * (varargin{I_all(n)+l} * z) );
                counter = counter + 1;
            end
        end
    end
    ...
    for iter=1:100
        d1=q-R{1}(z);
        d2=R{2}(z)+R{3}(z);
        lam=l2norm(d1)/(l2norm(d2)+1e-15)/2;
        z=z+d1+lam*d2;
    end
end
