...
net = createDiffusionNetwork3d3(imgSize,numInputChannels);
...
save('/home/ubuntu/Documents/work/train/diffisionnet/nnet/GenerateImagesUsingDiffusionExample/net/resnet3d64.mat',"net");
%%
numImages = 1;
figure
tic;generatedImages = generateAndDisplayImages(net,numDownSampSteps,imgSize,numImages);toc
%%
numImages = 1;
displayFrequency = 10;
numDownSampSteps=3;
...
figure
tic;generatedImages2 = generateAndDisplayImages2(net,param,numDownSampSteps,imgSize,numImages,1);toc
%%
...
%%
function images = generateAndDisplayImages(net,numDownSampSteps,imageSize,numImages)
...
for DownSampStep = numDownSampSteps:-1:1
    ...
    DownSampFactor=2^(numDownSampSteps-1);
    if DownSampStep~=1
...
%%
function images = generateAndDisplayImages2(net,param,numDownSampSteps,imageSize,numImages,iternum)
% Generate random noise.
...
for DownSampStep = numDownSampSteps:-1:1
    images_r = predict(net,real(images),DownSampStep);
    images_i = predict(net,imag(images),DownSampStep);
    images0=images_r+1i*images_i;
    images0 = regularizedReconstruction_dc(param.Fg,param.rawdata,param.P{:},'maxit',iternum,'verbose_flag', 0,'tol',1e-5,'z0',images0);
    if DownSampStep~=1
