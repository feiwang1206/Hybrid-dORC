...
mask=zeros(imgSize);
% mask(:,1:2:end)=1;
idx=randperm(prod(imgSize),floor(prod(imgSize)/4));
for i=1:length(idx)
...
if doTraining
    ...
    save('/home/ubuntu/Documents/work/train/diffisionnet/nnet/GenerateImagesUsingDiffusionExample/net/fftNet2d_mnist.mat',"net");
else
    load('/home/ubuntu/Documents/work/train/diffisionnet/nnet/GenerateImagesUsingDiffusionExample/net/fftNet2d_mnist.mat',"net");
end
%%
figure
...
function DownSampImg = applyDownSampToImage(img,DownSampFactor)
    ...
    for i=1:ss(3)
        for j=1:ss(4)
            ...
            imgf=imgf.*mask;
            DownSampImg(:,:,i,j)=real(ifft(ifft(fftshift(imgf),[],1),[],2));
        end
    end
end
%%
