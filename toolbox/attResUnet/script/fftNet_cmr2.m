...
if doTraining
    ...
    while epoch < numEpochs && ~monitor.Stop
        ...
        while hasdata(mbq) && ~monitor.Stop
            ...
            DownSampFactor = dlarray(randi(numDownSampSteps-1,[1 miniBatchSize],'Like',img)+1,"CB");

            % Apply noise to the image.
            DownSampImage1 = applyDownSampToImage_dl(img,DownSampFactor);
            DownSampImage2 = applyDownSampToImage_dl(img,DownSampFactor-1);

            % Compute loss.
            [loss,gradients] = dlfeval(@modelLoss,net,DownSampImage1,DownSampFactor,DownSampImage2-DownSampImage1);
    ...
    save('/home/ubuntu/Documents/work/train/diffisionnet/nnet/GenerateImagesUsingDiffusionExample/net/fftNet2d_cmr2.mat',"net");
else
    load('/home/ubuntu/Documents/work/train/diffisionnet/nnet/GenerateImagesUsingDiffusionExample/net/fftNet2d_cmr2.mat',"net");
end
%%
figure
...
for DownSampStep = numDownSampSteps:-1:1
    ...
    images = predict(net,images,DownSampStep)+images;
...
nexttile
imshow(abs(complex(images(:,:,1),images(:,:,2))),[]);colorbar
loss=l2norm(Img-images);
printf(['loss2=' num2str(loss)])
end
%%
