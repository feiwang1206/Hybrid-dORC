function net = createDiffusionNetwork3d_train(imagesize,inputChannels,outputChannels)
% CREATEDIFFUSIONNETWORK Create a network to predict the noise added to an
% image.

%   Copyright 2023 The MathWorks, Inc.

inputSize = [imagesize inputChannels];
...
layers = [
    ...
    swishLayer
    convolution3dLayer(filterSize,outputChannels,'Padding',"same");
...
layers = [
    ...
    SpatialFlattenLayer3d('Name',"flatten"+name)
    selfAttentionLayer(numHeads,numKeyChannels)
    SpatialUnflattenLayer3d('flatSize',flatSize,'Name',"unflatten"+name)
