...
for n=1:2
    ...
    if ~exist(fname,'file')
        ...
        for step=1:5
            ...
            % subplot(2,2,1),imagesc(array2mosaic(wmap_update.*mask-data.wmap0.*mask));colorbar,title(num2str(step))
            % subplot(2,2,2),imagesc(array2mosaic(imresize3D(recon{6},dim1).*mask-data.wmap0.*mask),[-50 50]);colorbar,title(num2str(step))
    ...
    if ~exist(fname,'file')
        ...
        for step=1:10
            ...
            % subplot(2,2,1),imagesc(array2mosaic(wmap_update.*mask-data.wmap0.*mask));colorbar,title(num2str(step))
            % subplot(2,2,2),imagesc(array2mosaic(imresize3D(recon{6},dim1).*mask-data.wmap0.*mask),[-20 20]);colorbar,title(num2str(step))
            % subplot(2,2,3),imagesc(array2mosaic(abs(recon{shot})));colorbar,title(num2str(step))
            % subplot(2,2,4),imagesc(array2mosaic(abs(recon00.recon{21})));colorbar,title(num2str(step))
            % drawnow;
        end
